...
import { useGetLegalDesclosureQuery } from "entries/legal/legal.generated";
...
const LegalDesclosure = () => {
  const localizationId = useLocalizationId();
  const { data } = useGetLegalDesclosureQuery(
    { localizationId: [localizationId] },
    { refetchOnWindowFocus: false, refetchOnMount: false }
  );
  const translations = useTranslations(["seo_impressum_description:p1"]);
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`${translations["seo_impressum_description"]}.`}
        />
      </Head>
...
export default LegalDesclosure;
